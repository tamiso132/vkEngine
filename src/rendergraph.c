...
//   //    This simulates the state of resources at the very beginning of the
//   frame
//   //    (before Pass 0).
...
//         // implicitly handled, BUT we usually emit one anyway to transition
//         from
...
//       // implementation, we barrier if the state is "dirty" or layout
//       differs.
//
//       if (need_barrier) {
//         if (is_image) {
...
//           b->subresourceRange =
//               (VkImageSubresourceRange){VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
//               1};
//         } else {
...
//       // No, standard practice: if we Read-After-Write, the hazard is
//       resolved.
...
//       // written. If we just read, keep the flag? Actually, once we barrier
//       for
...
//   // Persist the final layouts back to the RM so the next frame knows where
//   we
//   // left off.
//   for (uint32_t i = 0; i < MAX_RESOURCES; i++) {
//     RGResource *res = &rg->rm->resources[i];
