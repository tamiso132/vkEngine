...
static inline void window_init(TWindow* win, M_GPU* gpu, M_Resource* rm, int w, int h, const char* title, const char* swapchain_name) {
    win->width = w;
    win->height = h;
    ...
    swapchain_init(gpu, rm, &win->swapchain, win->surface, w, h, swapchain_name);
}

static inline void window_destroy(TWindow* win, M_GPU* gpu) {
    swapchain_destroy(gpu, &win->swapchain);
