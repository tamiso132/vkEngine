        ...
        float ao)
{
       ...
       if (debug_mode == DEBUG_MODE_ITER_GRAY) {
        float t = clamp(float(iter_count) / 256.0, 0.0, 1.0); // Normalize to expected cost (~256 steps)
        
        //Heatmap: Blue (low cost) -> Red (high cost)
        vec3 heatmap = mix(vec3(0,0,1), vec3(1,0,0), t);
        return vec4(heatmap, 1.0);
        }
