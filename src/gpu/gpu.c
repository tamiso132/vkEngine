...
bool gpu_init(GPUDevice* dev, GLFWwindow* window, GPUInstanceInfo* info) {
    ...
    if (vkCreateInstance(&instInfo, NULL, &dev->instance) != VK_SUCCESS) return false;
    volkLoadInstance(dev->instance);

    // 2. Surface
    if (window) glfwCreateWindowSurface(dev->instance, window, NULL, &dev->surface);
    ...
    // Enable features needed for RenderGraph (Dynamic Rendering, Synchronization2)
    VkPhysicalDeviceSynchronization2Features sync2 = { .sType=VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SYNCHRONIZATION_2_FEATURES, .synchronization2=VK_TRUE };
    VkPhysicalDeviceDynamicRenderingFeatures dynR = { .sType=VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DYNAMIC_RENDERING_FEATURES, .dynamicRendering=VK_TRUE, .pNext=&sync2 };
    VkPhysicalDeviceFeatures2 feats2 = { .sType=VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, .pNext=&dynR };
    ...
    if (vkCreateDevice(dev->physical_device, &dInfo, NULL, &dev->device) != VK_SUCCESS) return false;
    ...
    VkCommandBufferAllocateInfo cmdInfo = { .sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool=dev->imm_cmd_pool, .level=VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount=1 };
...
void gpu_immediate_submit(GPUDevice* dev, void (*callback)(VkCommandBuffer, void*), void* user_data) {
    ...
    VkSubmitInfo si = { .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO, .commandBufferCount = 1, .pCommandBuffers = &dev->imm_cmd_buffer };
...
bool gpu_swapchain_init(GPUDevice* dev, GPUSwapchain* sc, uint32_t w, uint32_t h) {
    ...
    if (vkCreateSwapchainKHR(dev->device, &ci, NULL, &sc->swapchain) != VK_SUCCESS) return false;
...
bool gpu_swapchain_acquire(GPUDevice* dev, GPUSwapchain* sc) {
    ...
    return vkAcquireNextImageKHR(dev->device, sc->swapchain, UINT64_MAX, sc->acquire_sem, VK_NULL_HANDLE, &sc->current_img_idx) == VK_SUCCESS;
}
...
void gpu_swapchain_present(GPUDevice* dev, GPUSwapchain* sc, VkQueue queue) {
    VkPresentInfoKHR pi = {
        ...
        .pWaitSemaphores = &sc->present_sem, // Must match what the RenderGraph signaled
        .swapchainCount = 1,
