...
#define NOT_IMPLEMENTED()                                                                                              \
  {                                                                                                                    \
    LOG_MESSAGE(CLR_RED, "ERROR", "NOT IMPLEMENTED");                                                                  \
    abort();                                                                                                           \
  }
...
typedef struct BitFlags32 {
  uint32_t bits;
} BitFlags32;

static inline bool bitflag_has(BitFlags32 f, uint32_t mask) { return (f.bits & mask) == mask; }

static inline void bitflag_set(BitFlags32 *f, uint32_t mask) { f->bits |= mask; }

static inline void bitflag_clear(BitFlags32 *f, uint32_t mask) { f->bits &= ~mask; }

static inline void bitflag_toggle(BitFlags32 *f, uint32_t mask) { f->bits ^= mask; }

static inline void bitflag_assign(BitFlags32 *f, uint32_t mask, bool value) {
  if (value)
    f->bits |= mask;
  else
    f->bits &= ~mask;
