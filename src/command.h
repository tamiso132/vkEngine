...
// PUBLIC FUNCTIONS
 ...
 void cmd_sync_image(CmdBuffer cmd, M_Resource *rm, ResHandle img_handle, ResourceState dst_state, AccessType dst_access);

 void cmd_buffer_upload(CmdBuffer cmd, M_GPU *dev, M_Resource *rm, ResHandle handle, void *data, u32 size);
 void cmd_sync_buffer(CmdBuffer cmd, M_Resource *rm, ResHandle buf_handle, ResourceState dst_state, AccessType dst_access);
