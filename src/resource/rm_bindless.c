...
void rm_bindless_shutdown(M_Resource *rm, M_GPU *gpu) {
  if (rm->default_sampler) vkDestroySampler(gpu->device, rm->default_sampler, NULL);
  if (rm->pip_layout) vkDestroyPipelineLayout(gpu->device, rm->pip_layout, NULL);
  if (rm->bindless_layout) vkDestroyDescriptorSetLayout(gpu->device, rm->bindless_layout, NULL);
  if (rm->descriptor_pool) vkDestroyDescriptorPool(gpu->device, rm->descriptor_pool, NULL);
