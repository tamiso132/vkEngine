...
// Simple Vector implementation (local to keep this file self-contained or use da.h)
typedef struct {
    WatchEntry* data;
    size_t count;
    size_t capacity;
...
void fw_add_watch(FileWatcher* fw, const char* path, FWCallback cb, void* user_data) {
    if (!fw || !path || !cb) return;

    // Check if we need to grow
    if (fw->watches.count >= fw->watches.capacity) {
        fw->watches.capacity *= 2;
        fw->watches.data = realloc(fw->watches.data, fw->watches.capacity * sizeof(WatchEntry));
    }
