...
static VkShaderModule _compile_glsl_glslang(VkDevice dev, const char *path,
...
void gp_register_hotreload(VkDevice device, GPUPipeline *target, GpBuilder *b) {
  if (!g_filewatcher)
    return;
  PipReloadCtx *ctx = malloc(sizeof(PipReloadCtx));
  ctx->device = device;
  ctx->target_pipeline = target;
  ctx->config = *b;
  if (b->vs_path)
    fw_add_watch(g_filewatcher, b->vs_path, _on_shader_changed, ctx);
  if (b->fs_path)
    fw_add_watch(g_filewatcher, b->fs_path, _on_shader_changed, ctx);
}
...
static VkShaderModule _compile_glsl_glslang(VkDevice dev, const char *path,
  ...
  // <--- CRITICAL FIX: Check if generation actually produced data before
  // passing to Vulkan
  if (!words || word_count == 0) {
    printf("[Glslang] Error: SPIR-V generation produced empty code for %s. "
           "Messages:\n%s\n",
...
static GPUPipeline _build_internal(VkDevice device, GpBuilder *b) {
  ...
  // 1. Compile Shaders
  VkShaderModule fs =
      _compile_glsl_glslang(device, b->fs_path, GLSLANG_STAGE_FRAGMENT);
  VkShaderModule vs =
      _compile_glsl_glslang(device, b->vs_path, GLSLANG_STAGE_VERTEX);
  // If VS failed, we can't continue.
  if (vs == VK_NULL_HANDLE) {
    printf("[Pipeline] Failed to compile Vertex Shader. Aborting build.\n");
    if (fs)
      vkDestroyShaderModule(device, fs,
                            NULL); // Cleanup if FS somehow succeeded
    return p;
  }
...
static void _on_shader_changed(const char *path, void *user_data) {
  PipReloadCtx *ctx = (PipReloadCtx *)user_data;
