...
static void _update_systems(SampleRunner *r, double dt) {
  ...
  if (input_mouse_pressed(&r->main_input, GLFW_MOUSE_BUTTON_LEFT)) {
    ivec2 mouse_pos = {};
    input_get_mouse_position(&r->main_input, r->ctx.dbg_mouse_pos);
  }
...
void run_sample(Sample *sample) {
  ...
  editor_pixel_editor_init(&editor, game_rect.size.width, game_rect.size.height, NULL);
  ReadBackBuffer readback = {};
  readback_init(&r.read_back, r.rm, game_rect.size);
